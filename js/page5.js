function goToPage5() {
    ...
    var boat1c = paper.path("M80,212 L62,202 L65,215 L80,212").attr({stroke: "#777", fill: "none"});
    ...
    function up1(e, g) {
        if (index < 4) {
            ...
            }, 2000, mina.linear, down1.bind(null, e, g));

        } else {
            e.animate({transform: 't' + progressX[index] + ',' + progressY[index] + ' r0 s' + scale1[index]}, 2000, mina.linear, up1.bind(null, e, g));
            g.animate({transform: 't' + progressX[index] + ',' + progressY[index] + ' r0 s' + scale3[index]}, 2000, mina.linear);
        }
    }
    ...
    function down1(e, g) {
        if (index < 5) {
            ...
            Snap.animate([progressX[index - 1], progressY[index - 1], rotate[index-1], scale1[index - 1], scale3[index - 1]], [progressX[index], progressY[index], rotate[index], scale1[index], scale3[index]], function (value) {
                e.attr({transform: 't' + value[0] + ',' + value[1] + ' r' + value[2] + ' s' + value[3]});
                g.attr({transform: 't' + value[0] + ',' + value[1] + ' r' + value[2] + ' s' + value[4]});
            }, 2000, mina.linear, up1.bind(null, e, g));

        } else {
            e.animate({transform: 't' + progressX[index] + ',' + progressY[index] + ' r6 s' + scale1[index]}, 2000, mina.linear, up1.bind(null, e, g));
            g.animate({transform: 't' + progressX[index] + ',' + progressY[index] + ' r6 s' + scale3[index]}, 2000, mina.linear);
        }
    }
}
